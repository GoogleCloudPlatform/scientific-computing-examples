...
if [ "X${arm_machine_type}" != "X" ]; then
    ...
    gcloud builds submit --config=computebuild.yaml --substitutions=_ZONE=${zone},_MACHINE_ARCHITECTURE="arm64",_MACHINE_TYPE=${arm_machine_type},_SOURCE_IMAGE=${source_image} .
fi

if [ "X${x86_machine_type}" != "X" ]; then
    source_image=$(gcloud compute images list --filter="name ~ rocky-linux-${rocky_linux_version}-optimized-gcp-v" --format="value(name)")
    gcloud builds submit --config=computebuild.yaml --substitutions=_ZONE=${zone},_MACHINE_ARCHITECTURE="x86-64",_MACHINE_TYPE=${x86_machine_type},_SOURCE_IMAGE=${source_image} .
fi
