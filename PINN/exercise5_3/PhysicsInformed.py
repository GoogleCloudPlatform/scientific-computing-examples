...
class PhysicsInformedBarModel:
    ...
    def costFunction(self, x, u_pred):
        ...
        # Differential equation cost
        differential_equation_cost = utilities.getDerivative(
            self.E(x) * self.A(x) * utilities.getDerivative(u_pred, x, 1), x, 1) + self.dist_load(x)
        differential_equation_cost = torch.sum(differential_equation_cost ** 2).view(1)

        # Boundary condition cost initialization
        boundary_condition_cost = 0

        # Sum over dirichlet boundary condition costes
        for i in range(len(self.uB)):
            boundary_condition_cost += (utilities.getDerivative(u_pred, x, self.uB[i][1])[self.uB[i][2]] - self.uB[i][
                0]) ** 2
    ...
    def closure(self):
        ...
        loss = differential_equation_cost + boundary_condition_cost * self.bc_weight
        loss.backward(retain_graph=True)
        return loss

    def train(self, epochs, optimizer, bc_weight=1e0, **kwargs):
        """Train the model."""
        ...
        for i in range(epochs):
            ...
            # Total cost
            total_cost = differential_equation_cost + boundary_condition_cost * bc_weight
