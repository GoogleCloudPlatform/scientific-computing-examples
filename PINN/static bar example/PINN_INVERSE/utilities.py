...
def plot_displacements_bar(x, u, u_analytic=None):
    ...
    # Plot data
    if u_analytic != None:
        ax.plot(x.detach().numpy(), u_analytic(x.detach().numpy()),color='r', label="$u_{\mathrm{analytic}}$")
    
    ax.plot(x.detach().numpy(), u.detach().numpy(), ':',label="$u_{\mathrm{pred}}$")
...
def plot_stiffness_bar(x, EA, EA_analytic=None):
    ...
    # Plot data
    if EA_analytic != None:
        ax.plot(x.detach().numpy(), EA_analytic(x.detach().numpy()),color='r', linewidth=2, label="$EA_{\mathrm{analytic}}$")
    ax.plot(x.detach().numpy(), EA.detach().numpy(),color='k',linestyle=':',linewidth=5, label="$EA_{\mathrm{pred}}$")
